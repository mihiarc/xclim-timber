...
class ClimateCSVFormatter:
    ...
    def __init__(self, historical_years: tuple = (1950, 2014)):
        ...
        # Auto-detect climate indices from data (expanded for MVP's 50+ indices)
        self.climate_indices = None  # Will be auto-detected from first CSV file
    ...
    def load_historical_data(self, data_dir: str) -> pd.DataFrame:
        ...
        # Check main output directory for various patterns
        csv_files.extend(list(data_path.glob('parcel_indices_[0-9]*.csv')))
        csv_files.extend(list(data_path.glob('*_indices_*.csv')))
        csv_files.extend(list(data_path.glob('climate_*.csv')))
    ...
    def create_wide_format(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Auto-detect climate indices from the dataframe
        if self.climate_indices is None:
            excluded_cols = location_cols + ['year']
            available_indices = [col for col in df.columns if col not in excluded_cols]
            self.climate_indices = available_indices
        else:
            available_indices = [col for col in self.climate_indices if col in df.columns]

        logger.info(f"Available climate indices ({len(available_indices)}): {available_indices[:10]}...")  # Show first 10
